...
def _write_providers_file(ctx, tgt, text):
    ...
    if CcSharedLibraryInfo in tgt:
        text = text + "{color}CcSharedLibraryInfo in {lbl}{reset}\n".format(
            color = CCCYN, reset = CCRESET, lbl=tgt.label)
        print("CCSHAREDLIBINFO: %s" % tgt[CcSharedLibraryInfo])
        print("CCSHARDLIBINFO: in rule %s" % tgt.label)
        # dump_compilation_context(tgt[CcInfo])
        text = text + ccsharedlibinfo_to_string(ctx, tgt[CcSharedLibraryInfo])
...
def _inspect_impl(ctx):
    ...
    if ctx.label.package == "inspect":
        if ctx.label.name == "import":
            ...
            tmp = ctx.attr.obj[0]
            objs = []
            if OCamlImportProvider in tmp:
                print("OCamlImportProvider: %s" % tmp)
        ...
        if ctx.label.name == "providers":
            ...
            if OCamlLibraryProvider in ctx.attr.obj:
