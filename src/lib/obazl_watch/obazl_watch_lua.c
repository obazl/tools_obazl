...
void l_on_change (fsw_cevent *event)
{
    ...
    /* create lua evt table (on tos) */
    lua_newtable(L);
    lua_pushstring(L, event->path);
    lua_setfield(L, -2, "path");

    /* WARNING! lua uses number, not integer for time, because not all
       platforms have 32-bit ints; but we assume OCaml development
       will only occur on machines with at least 32-bit ints. */
    /* see http://lua-users.org/lists/lua-l/2006-01/msg00143.html */
    log_debug("evt time: %d", event->evt_time);
    lua_pushinteger(L, event->evt_time); /* instead of lua_pushnumber */
    lua_setfield(L, -2, "time");

    /* set the flags */
    lua_newtable(L);
    for (int i = 0; i < event->flags_num; i++) {
        log_debug("\tflag: %d: %s",
                  event->flags[i],
                  fsw_get_event_flag_name(event->flags[i]));
        lua_pushinteger(L, event->flags[i]);
        /* flag = fsw_get_event_flag_name(event->flags[i]); */
        lua_setfield(L, -2, fsw_get_event_flag_name(event->flags[i]));
    }
    lua_setfield(L, -2, "flags");

    /* now get the user-provided fn */

    /* lua file 'obazl_watch.lua' is already loaded, by config  */
    lua_getglobal(L, "bazel");  /* bazel table */
    int ftype = lua_getfield(L, -1, "on_fs_event");
    if (ftype == LUA_TNIL) {
        log_warn("User did not provide an implementation for 'on_fs_event'");
        /* user did not provide 'on_fs_event' implementation; ignore */
        return;
    }
    if ( !lua_isfunction(L, -1) ) {
        log_fatal("getfield on_fs_event did not return a fn: %s", lua_typename(L, ftype));
        exit(EXIT_FAILURE);
    }

    /* stack: [tevt tbzl fn_on_fs_event] */
    lua_rotate(L, -3, 2);
    /* stack: [tbzl fn_on_fs_event tevt] */
    /* verify */
    if ( !lua_istable(L, -1) ) {
        log_fatal("Oops, rotate did not work: %s", lua_typename(L, lua_type(L, -1)));
        exit(EXIT_FAILURE);
    }
    log_debug("calling user 'on_fs_event'");
    /* do the call (1 argument, 0 results) */
    if (lua_pcall(L, 1, 0, 0) != 0) {
        log_error("error running lua fn: on_fs_event");
        lerror(L, "error running function `on_fs_event': %s", lua_tostring(L, -1));
    }
