...
(define (-deps-kw->cmd-args deps)
  ...
  (let ((args (map (lambda (dep)
                     (if *mibl-debugging* (format #t "dep: ~A\n" dep))
                     (case (car dep)
                       ((::) ;; local files, :foo.txt
                        (map (lambda (d)
...
(define (-arg->dep arg deps)
  ...
  (let* ((dlist (find-if (lambda (deplist)
                           (if *mibl-debugging* (format #t "deplist: ~A~%" deplist))
                           (case (car deplist)
                             ((::)
         ...
         (dep (if dlist
                  (begin
                    (if *mibl-debugging* (format #t "found dlist: ~A~%" dlist))
                    (find-if (lambda (d)
                               (if *mibl-debugging* (format #t "finding?: ~A~%" d))
                               (string-suffix? arg d))
                             (cdr dlist)))
                  #f)))
...
(define (-deps->args deps)
  ...
  (map (lambda (dep) 
        (if *mibl-debugging* (format #t "~A: ~A~%" (blue "dep") dep))
         (if (eq? ::tools (car dep))
             (values)
...
(define (-resolve-args cmd pkg-path deps outputs)
  ...
  (let* ((-args (assoc-val :args (cdr cmd)))
         (_ (if *mibl-debugging* (format #t "-args: ~A~%" -args)))
         (args
          (map (lambda (arg)
                 (if *mibl-debugging* (format #t "Mapping arg: ~A~%" arg))
                 (cond
                  ((or (equal? arg :targets)
                       (equal? arg :outputs))
                   (if *mibl-debugging* (format #t "~A: ~A~%" (red "Arg is") arg))
                  ...
                  ((keyword? arg)
                   ...
                   (if (equal? :deps arg)
                       ...
                       (if (equal? ::pkg-dir arg)
                           ...
                           (let ((found
                                  (find-if (lambda (dep)
                                             (if *mibl-debugging* (format #t "checking dep: ~A\n" dep))
                                             (equal? arg (car dep)))
                                           deps)))
                             (if found
                                 (begin
                  ...
                  ((string? arg) ;; e.g. a file literal
                   (if *mibl-debugging* (format #t "arg: string literal\n"))
                   (if (char=? #\- (arg 0))
                       arg
                   ...
                   ;; strings we see are just string args
                       (if-let ((x (-arg->dep arg deps)))
                               ;; (assoc arg deps)))
                               (begin
                                 (if *mibl-debugging* (format #t "found arg ~A in deps\n" x))
                               ...
                               (begin
                                 (if *mibl-debugging* (format #t "arg not in deps: ~A~%" arg))
                                 arg))))
               ...
               -args)))
    (if *mibl-debugging* (format #t "~A: ~A~%" (red "Args") args))
    args))
...
(define (derive-cmd pkg-path cmd deps targets)
 ...
 ;; run: (:cmd (:tool (:pkg :bin) (:tgt foo)) ...)
  ...
  (let* ((cmd-alist (cdr cmd))
         ...
         (_ (if *mibl-debugging* (format #t "tool-dep?: ~A~%" tool-dep?)))
...
(define (starlark-emit-copy-file outp pkg-path stanza)
  ...
  (let* ((output (assoc-val :outputs (cdr stanza)))
         (_ (if *mibl-debugging* (format #t "~A: ~A~%" (yellow "output") output)))
         (output-alist (cdar output))
         (_ (if *mibl-debugging* (format #t "~A: ~A~%" (yellow "output-alist") output-alist)))
         (out-pkg (assoc-val :pkg output-alist))
         (_ (if *mibl-debugging* (format #t "~A: ~A~%" (yellow "out-pkg") out-pkg)))
         (_ (if *mibl-debugging* (format #t "~A: ~A~%" (yellow "pkg-path") pkg-path)))
         (out-tgt (assoc-val :tgt output-alist))
         (_ (if *mibl-debugging* (format #t "~A: ~A~%" (yellow "out-tgt") out-tgt)))

         (deps (assoc-val :deps stanza))
         (_ (if *mibl-debugging* (format #t "~A: ~A~%" (uwhite "deps") deps)))

         (cmd-args (cdr (assoc-in '(:actions :cmd :args) stanza)))
         (_ (if *mibl-debugging* (format #t "~A: ~A~%" (uwhite "cmd-args") cmd-args)))
         (src-arg (if (equal? :deps (car cmd-args))
                      deps
                      (assoc-val (car cmd-args) deps)))
         (_ (if *mibl-debugging* (format #t "~A: ~A~%" (ured "src-arg") src-arg)))

         (in-pkg (assoc-val :pkg src-arg))
         (_ (if *mibl-debugging* (format #t "~A: ~A~%" (yellow "in-pkg") in-pkg)))
         ...
         (_ (if *mibl-debugging* (format #t "~A: ~A~%" (yellow "in-tgt") in-tgt)))
    ...
    (if (equal? out-pkg pkg-path)
        ...
        (let ((out  (format #f "~A" tgt))
              ...
              (_ (if *mibl-debugging* (format #t "~A: ~A~%" (yellow "args") args)))
...
(define (starlark-emit-write-file-target outp stanza)
  ...
  (let* ((output (assoc-val :outputs (cdr stanza)))
        (_ (if *mibl-debugging* (format #t "~A: ~A~%" (yellow "output") output)))
        (pkg-tgt (cdar output))
        (_ (if *mibl-debugging* (format #t "~A: ~A~%" (yellow "pkg-tgt") pkg-tgt)))
        (pkg (assoc-val :pkg pkg-tgt))
        (_ (if *mibl-debugging* (format #t "~A: ~A~%" (yellow "pkg") pkg)))
        (tgt (assoc-val :tgt pkg-tgt))
        (_ (if *mibl-debugging* (format #t "~A: ~A~%" (yellow "tgt") tgt)))
        (out  (format #f "~A" tgt))
        (name (format #f "__~A__" tgt))
        (args (assoc-in '(:actions :cmd :args) (cdr stanza)))
        (_ (if *mibl-debugging* (format #t "~A: ~A~%" (yellow "args") args)))
...
(define (starlark-emit-cppo-target outp stanza pkg)
  ...
  (let* ((pkg-path (car (assoc-val :pkg-path pkg)))
         ...
         (_ (if *mibl-debugging* (format #t "~A: ~A~%" (yellow "name") name)))
         ...
         (cmd-outputs (map (lambda (out) (format #f "$(location ~A)" out))
                           outputs))
         (_ (if *mibl-debugging* (format #t "~A: ~A~%" (yellow "cmd-outputs") cmd-outputs)))
                        ...
                        (map (lambda (src)
                               (let ((dn (dirname src)))
                                 (if (string=? "./" dn)
                                     ...
                                     dn)))
                             srcs)))
         (_ (if *mibl-debugging* (format #t "~A: ~A~%" (uyellow "include") include-dirs)))
         (args (assoc-in '(:actions :cmd :args) (cdr stanza)))
         (_ (if *mibl-debugging* (format #t "~A: ~A~%" (yellow "args") args)))
         (args (flatten
                (map (lambda (arg)
                       ...
                       (if-let ((a (assoc-val arg deps)))
                               (let ((_ (if *mibl-debugging* (format #t "~A: ~A~%" (yellow "A") a)))
                               ...
                               (if (equal? :outputs arg)
                                   ...
                                   arg)))
                     (cdr args)))))
    (if *mibl-debugging* (format #t "~A: ~A~%" (yellow "args") args))
    ;; (error 'x "x")
    ;; (rule
    ;;  (targets yojson.mli)
    ;;  (deps
...
(define (starlark-emit-rule-target outp pkg-path stanza)
  ...
  (for-each (lambda (cmd)
              (if *mibl-debugging* (format #t "Rule cmd: ~A~%" cmd))
              (if (eq? :cmd (car cmd))
                  (let* ((tool (assoc-val :tool (cdr cmd)))
                         (_ (if *mibl-debugging* (format #t "tool: ~A~%" tool))))
                    (case (car tool)
                      ((:write-file)
...
(define (starlark-emit-diff-target outp pkg-path stanza-alist)
  ...
  (let* ((action (assoc-val :actions stanza-alist))
         (_ (if *mibl-debugging* (format #t "~A: ~A~%" (uwhite "action") action)))
         ...
         (_ (if *mibl-debugging* (format #t "~A: ~A~%" (cyan "deps") deps)))
         (srcs (deps->srcs-attr pkg-path tool deps))
         (_ (if *mibl-debugging* (format #t "~A: ~A~%" (cyan "genrule srcs") srcs)))
         ...
         (_ (if *mibl-debugging* (format #t "~A: ~A~%" (ucyan "with-stdout?") with-stdout?)))
         (outputs (assoc-val :outputs stanza-alist))
         (_ (if *mibl-debugging* (format #t "~A: ~A~%" (ucyan "outputs") outputs)))
         (outs (if outputs (outputs->outs-attr pkg-path outputs) '()))
         (_ (if *mibl-debugging* (format #t "~A: ~A~%" (ucyan "outs") outs)))
    ...
    (for-each
     (lambda (cmd)
       (if (eq? :cmd (car cmd))
           (if *mibl-debugging* (format #t "GENRULE ACTION (B): ~A~%" cmd))
           (if (eq? :stdout (car cmd))
               (if *mibl-debugging* (format #t "GENRULE STDOUT (B): ~A~%" cmd))
    ...
    (if-let ((ctx (assoc-in '(:actions :ctx) stanza-alist)))
            ...
            ;; else
            (begin
              (if *mibl-debugging* (format #t "  outs: ~A~%" outs))
...
(define (starlark-emit-node-target outp pkg-path stanza-alist)
  ...
  (let* ((action (assoc-val :actions stanza-alist))
         ...
         (_ (if *mibl-debugging* (format #t "~A: ~A~%" (cyan "node args") args)))
