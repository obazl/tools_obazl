  ...
  (let* ((stanza-alist (cdr stanza))
         ...
         (tools (let* ((tool (cadr
                  ...
                  ;;      (pkg-path (assoc-val :pkg (cdr ts)))
                       ;; (_ (if *mibl-debugging* (format #t "~A: ~A~%" (uwhite "pkg-path")
                       ;;            pkg-path)))
                  ;;      (tgt (assoc-val :tgt (cdr ts)))
                  ;;      (_ (if *mibl-debugging* (format #t "~A: ~A~%" (uwhite "tgt") tgt))))
         ...
         (_ (if *mibl-debugging* (format #t "~A: ~A~%" (uwhite "cooked deps") deps)))

         (args (cdr (assoc-in '(:actions :cmd :args) stanza-alist)))
         (_ (if *mibl-debugging* (format #t "~A: ~A~%" (uwhite "args") args)))
         (args (-decode-sh-test-args args ws pkg stanza))
         (_ (if *mibl-debugging* (format #t "~A: ~A~%" (uwhite "Cooked args") args)))
         )
    (if *mibl-debugging*
        (format #t "TARGET: ~A\n" pubname))
...
;;          (_ (if *mibl-debugging* (format #t "main: ~A~%" main)))

;;          ;; (deps (assoc-in '(:compile :deps)))
;;          ;; (_ (if *mibl-debugging* (format #t "compile deps: ~A~%" deps)))
;;          (manifest (sort! (if-let ((mani
;;                              (assoc-in '(:link :manifest :modules)
;;                                        stanza-alist)))
;;                                  (cdr mani) '())
;;                           sym<?))
;;          (_ (if *mibl-debugging* (format #t "manifest: ~A~%" manifest)))
