  ...
  (let* ((pkg-path (car (assoc-val :pkg-path pkg)))
    ...
    (if (and dunefile (not (null? (cdr dunefile))))
        (let* ((stanzas (cadr dunefile))
          ...
          (if *local-ppx-driver*
              (begin
                (if *debugging* (format #t "emitting pkg ppxes\n"))
          ...
          (if *debugging* (format #t "emitting conditional deps\n"))
