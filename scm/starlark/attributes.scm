...
(define (deps->srcs-attr pkg-path tool deps)
  ...
  (if deps
      (let* ((srcs (map (lambda (dep)
                          (if *mibl-debug-s7* (format #t "dep: ~A~%" dep))
                          (cond
                           ...
                           (else
                            (let* ((tag (car dep))
                                   (_ (if *mibl-debug-s7* (format #t "~A: ~A~%" (yellow "tag") tag)))
                                   (label (cdr dep))
                                   (_ (if *mibl-debug-s7* (format #t "~A: ~A~%" (yellow "label") label))))
                              (cond
                               ((alist? label)
                        ...
                        deps))
             (_ (if *mibl-debug-s7* (format #t "~A: ~A~%" (uwhite "prelim srcs") srcs)))
