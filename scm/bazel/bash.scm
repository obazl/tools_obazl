        ...
        (let* ((key (string->keyword tool)))
          (let* ((deps (assoc-val :deps stanza))
                 ...
                 (match (find-if (lambda (dep)
                                   ...
                                   (eq? key (car dep)))
                                 deps)))
            (if (or *mibl-debug-genrules* *mibl-debug-s7*) (format #t "~A: ~A~%" (yellow "match") match))
            (if match
                (let* ((lbl (cdr match))
                  ...
                  (if (or *mibl-debug-genrules* *mibl-debug-s7*) (format #t "~A: ~A~%" (yellow "RESOLVED") tgt))
                  tgt)
                ;; (format #t "~A~%" tool)
                ))))))
...
(define (-expand-bash-args args pkg-path stanza)
  (if (or *mibl-debug-genrules* *mibl-debug-s7*) (format #t "~A: ~A~%" (ublue "-expand-bash-args") args))
    ...
    (if (truthy? args)
        (let-values (((tool parsed-args)
          ...
          (let ((srcs (remove tool srcs)))
            (if (or *mibl-debug-genrules* *mibl-debug-s7*) (format #t "~A: ~A~%" (red "srcs") srcs))
            (format outp "    srcs  = [\n")
            (format outp "~{        \"~A\"~^,\n~}\n" srcs)
            (format outp "    ],\n")))
        (begin
