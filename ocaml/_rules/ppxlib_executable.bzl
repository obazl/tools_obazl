...
###########################
def _ppxlib_executable(ctx):

    print("{c}ppxlib_executable: {m}{r}".format(
...
########## DECL:  PPXLIB_EXECUTABLE  ################
ppxlib_executable = rule(
    implementation = _ppxlib_executable,
    ...
    attrs = dict(
        ...
        # initializers = attr.label_list(),
        # finalizers = attr.label_list(),

        ## ppxlib always uses same main, which we supply
        main = attr.label(
        ...
        # FIXME: no need for ppx attrib on ppxlib_executable?
        ...
        #     doc = "List of args that will be passed to the ppxlib_executable at runtime. E.g. -inline-test-lib. CAVEAT: will be used wherever the exec is run, and passed before command line args.  For finer granularity use the 'ppx_args' attr of e.g. ocaml_module."
        # ),
