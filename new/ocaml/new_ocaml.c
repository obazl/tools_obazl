...
char **template;
int ocaml_pfx_len = 29; // strlen("external/obazl/new/templates") + 1
...
EXPORT void new_ocaml_package(char *pgm)
{
    log_debug("NEW_ocaml_package: %s", pgm);
}

EXPORT void new_ocaml_module(char *pgm)
{
    log_debug("NEW_ocaml_module: %s", pgm);
    return;
    /* if (debug) { */
    ...
    while (strlen(*template) != 0) {
        ...
        dest = strndup(*template + ocaml_pfx_len,
                       strlen(*template) - ocaml_pfx_len);
...
/*     int i = 0; */
/*     int ocaml_pfx_len = 20; // "obazl/new/templates" */
...
/*                 dest = strndup(runfiles[i].key + ocaml_pfx_len, */
/*                                strlen(runfiles[i].key) - ocaml_pfx_len); */
