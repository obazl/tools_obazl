...
(define (-arg->dep arg deps)
  ...
  (let* ((dlist (find-if (lambda (deplist)
                           (if *debugging* (format #t "deplist: ~A~%" deplist))
                           (case (car deplist)
                             ((::)
         ...
         (dep (if dlist
                  (begin
                    (if *debugging* (format #t "found dlist: ~A~%" dlist))
                    (find-if (lambda (d)
                               (if *debugging* (format #t "finding?: ~A~%" d))
                               (string-suffix? arg d))
                             (cdr dlist)))
                  #f)))
...
(define (-resolve-args cmd pkg-path deps outputs)
  ...
  (let* ((-args (assoc-val :args (cdr cmd)))
         (_ (if *debugging* (format #t "-args: ~A~%" -args)))
         (args
          (map (lambda (arg)
                 (if *debugging* (format #t "Mapping arg: ~A~%" arg))
                 (cond
                  ((or (equal? arg :targets)
                       (equal? arg :outputs))
                   (if *debugging* (format #t "~A: ~A~%" (red "Arg is") arg))
                  ...
                  ((string? arg) ;; e.g. a file literal
                   (if *debugging* (format #t "arg: string literal\n"))
                   (if (char=? #\- (arg 0))
                       arg
                   ...
                   ;; strings we see are just string args
                       (if-let ((x (-arg->dep arg deps)))
                               ;; (assoc arg deps)))
                               (begin
                                 (if *debugging* (format #t "found arg ~A in deps\n" x))
                               ...
                               (begin
                                 (if *debugging* (format #t "arg not in deps: ~A~%" arg))
                                 arg))))
               ...
               -args)))
    (if *debugging* (format #t "~A: ~A~%" (red "Args") args))
    args))
...
(define (starlark-emit-copy-file outp pkg-path stanza)
  ...
  (let* ((output (assoc-val :outputs (cdr stanza)))
         (_ (if *debugging* (format #t "~A: ~A~%" (yellow "output") output)))
         (output-alist (cdar output))
         (_ (if *debugging* (format #t "~A: ~A~%" (yellow "output-alist") output-alist)))
         (out-pkg (assoc-val :pkg output-alist))
         (_ (if *debugging* (format #t "~A: ~A~%" (yellow "out-pkg") out-pkg)))
         (_ (if *debugging* (format #t "~A: ~A~%" (yellow "pkg-path") pkg-path)))
         (out-tgt (assoc-val :tgt output-alist))
         (_ (if *debugging* (format #t "~A: ~A~%" (yellow "out-tgt") out-tgt)))

         (deps (assoc-val :deps stanza))
         (_ (if *debugging* (format #t "~A: ~A~%" (uwhite "deps") deps)))

         (cmd-args (cdr (assoc-in '(:actions :cmd :args) stanza)))
         (_ (if *debugging* (format #t "~A: ~A~%" (uwhite "cmd-args") cmd-args)))
         (src-arg (if (equal? :deps (car cmd-args))
                      deps
                      (assoc-val (car cmd-args) deps)))
         (_ (if *debugging* (format #t "~A: ~A~%" (ured "src-arg") src-arg)))

         (in-pkg (assoc-val :pkg src-arg))
         (_ (if *debugging* (format #t "~A: ~A~%" (yellow "in-pkg") in-pkg)))
         ...
         (_ (if *debugging* (format #t "~A: ~A~%" (yellow "in-tgt") in-tgt)))
    ...
    (if (equal? out-pkg pkg-path)
        ...
        (let ((out  (format #f "~A" tgt))
              ...
              (_ (if *debugging* (format #t "~A: ~A~%" (yellow "args") args)))
...
(define (starlark-emit-write-file-target outp stanza)
  ...
  (let* ((output (assoc-val :outputs (cdr stanza)))
        (_ (if *debugging* (format #t "~A: ~A~%" (yellow "output") output)))
        (pkg-tgt (cdar output))
        (_ (if *debugging* (format #t "~A: ~A~%" (yellow "pkg-tgt") pkg-tgt)))
        (pkg (assoc-val :pkg pkg-tgt))
        (_ (if *debugging* (format #t "~A: ~A~%" (yellow "pkg") pkg)))
        (tgt (assoc-val :tgt pkg-tgt))
        (_ (if *debugging* (format #t "~A: ~A~%" (yellow "tgt") tgt)))
        (out  (format #f "~A" tgt))
        (name (format #f "__~A__" tgt))
        (args (assoc-in '(:actions :cmd :args) (cdr stanza)))
        (_ (if *debugging* (format #t "~A: ~A~%" (yellow "args") args)))
...
(define (starlark-emit-cppo-target outp stanza pkg)
  ...
  (let* ((pkg-path (car (assoc-val :pkg-path pkg)))
         ...
         (_ (if *debugging* (format #t "~A: ~A~%" (yellow "outputs") outputs)))
         (name (assoc-val :tgt (cdr (outputs 0))))
         (_ (if *debugging* (format #t "~A: ~A~%" (yellow "name") name)))
         ...
         (cmd-outputs (map (lambda (out) (format #f "$(location ~A)" out))
                           outputs))
         (_ (if *debugging* (format #t "~A: ~A~%" (yellow "cmd-outputs") cmd-outputs)))
                        ...
                        (map (lambda (src)
                               (let ((dn (dirname src)))
                                 (if (string=? "./" dn)
                                     ...
                                     dn)))
                             srcs)))
         (_ (if *debugging* (format #t "~A: ~A~%" (uyellow "include") include-dirs)))
         (args (assoc-in '(:actions :cmd :args) (cdr stanza)))
         (_ (if *debugging* (format #t "~A: ~A~%" (yellow "args") args)))
         (args (flatten
                (map (lambda (arg)
                       ...
                       (if-let ((a (assoc-val arg deps)))
                               (let ((_ (if *debugging* (format #t "~A: ~A~%" (yellow "A") a)))
                               ...
                               (if (equal? :outputs arg)
                                   ...
                                   arg)))
                     (cdr args)))))
    (if *debugging* (format #t "~A: ~A~%" (yellow "args") args))
    ;; (error 'x "x")
    ;; (rule
    ;;  (targets yojson.mli)
    ;;  (deps
...
(define (starlark-emit-rule-target outp pkg-path stanza)
  ...
  (for-each (lambda (cmd)
              (if *debugging* (format #t "Rule cmd: ~A~%" cmd))
              (if (eq? :cmd (car cmd))
                  (let* ((tool (assoc-val :tool (cdr cmd)))
                         (_ (if *debugging* (format #t "tool: ~A~%" tool))))
                    (case (car tool)
                      ((:write-file)
...
(define (starlark-emit-diff-target outp pkg-path stanza-alist)
  ...
  (let* ((action (assoc-val :actions stanza-alist))
    ...
    (for-each
     (lambda (cmd)
       (if (eq? :cmd (car cmd))
           (if *debugging* (format #t "GENRULE ACTION (B): ~A~%" cmd))
           (if (eq? :stdout (car cmd))
               (if *debugging* (format #t "GENRULE STDOUT (B): ~A~%" cmd))
...
(define (starlark-emit-node-target outp pkg-path stanza-alist)
  ...
  (let* ((action (assoc-val :actions stanza-alist))
         ...
         (_ (if *debugging* (format #t "~A: ~A~%" (cyan "node args") args)))
